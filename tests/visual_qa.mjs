...
async function inspect(page,name){
  ...
  if(d.minCoreFont!==null&&d.minCoreFont<9.5)err(name+': core text too small '+d.minCoreFont+'px');
...
async function desktop(browser){
  ...
  const e0=await page.locator('#elapsed').innerText(),o0=await page.locator('[data-qa="observation-title"]').innerText();
  await page.waitForTimeout(450);
  const e1=await page.locator('#elapsed').innerText(),o1=await page.locator('[data-qa="observation-title"]').innerText();
  report.interactions.live={elapsed0:e0,elapsed1:e1,plan0:o0,plan1:o1};
  if(e0===e1)err('desktop: simulation clock did not advance');
  if(o0===o1)warn('desktop: plan number did not advance over 450 ms');
...
async function mobile(browser){
  ...
  if(sc.scrollWidth>sc.clientWidth+2&&!['auto','scroll'].includes(sc.overflowX))err('mobile: internal denoise sequence overflow is not scrollable');
